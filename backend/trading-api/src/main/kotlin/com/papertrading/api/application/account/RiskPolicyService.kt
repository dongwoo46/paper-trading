...
class RiskPolicyService(
    ...
    @Transactional
    fun replaceRiskPolicy(accountId: Long, command: UpsertRiskPolicyCommand): RiskPolicy {
    ...
    fun getActiveRiskPolicy(accountId: Long): RiskPolicy =
        riskPolicyRepository.findByAccountIdAndIsActiveTrue(accountId)
            .orElseThrow { NotFoundException("RISK_POLICY_NOT_FOUND", "활성 리스크 정책이 없습니다. accountId=$accountId") }
}
