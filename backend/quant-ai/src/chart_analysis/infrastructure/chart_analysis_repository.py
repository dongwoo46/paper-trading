...
class PostgresChartAnalysisRepository:
    ...
    def find_by_symbol(self, symbol: str) -> list[ChartAnalysisResult]:
        ...
        query = (
            ...
            "llm_report, llm_report_source, template_report, numeric_computed_at, llm_computed_at "
    ...
    def _to_params(self, result: ChartAnalysisResult) -> tuple:
        ...
        template_report_json = None
        if result.template_report is not None:
            template_report_json = _dumps({
                "trend_section": result.template_report.trend_section,
                "support_resistance_section": result.template_report.support_resistance_section,
                "entry_plan_section": result.template_report.entry_plan_section,
                "signal_evidence_section": result.template_report.signal_evidence_section,
                "risk_section": result.template_report.risk_section,
                "source": result.template_report.source.value,
            })
    ...
    def _row_to_result(row: tuple) -> ChartAnalysisResult:
        (
            ...
            llm_report_raw, llm_report_source_str, template_report_raw, numeric_computed_at, llm_computed_at,
        ) = row
        ...
        if template_report_raw is not None:
            tmpl_data = _parse_json(template_report_raw)
            template_report = NarrativeReport(
                trend_section=tmpl_data["trend_section"],
                support_resistance_section=tmpl_data["support_resistance_section"],
                entry_plan_section=tmpl_data["entry_plan_section"],
                signal_evidence_section=tmpl_data["signal_evidence_section"],
                risk_section=tmpl_data["risk_section"],
                source=ReportSource(tmpl_data.get("source", "rule_template")),
            )
...
ON CONFLICT (symbol, "window", "interval") DO UPDATE SET
    ...
    template_report     = EXCLUDED.template_report,
...
def _build_order_case() -> str:
    """Stable ordering for the supported analysis windows."""
    parts = []
    for i, (w, iv) in enumerate(_WINDOW_ORDER):
        parts.append(f"WHEN \"window\" = '{w}' AND \"interval\" = '{iv}' THEN {i}")
    return " ".join(parts)
