...
class PopularSymbolsRefreshService:
    ...
    def _compute_scores(metrics: list[dict]) -> dict[str, Decimal]:
        ...
        caps = [float(m["market_cap"]) for m in metrics]
        vols = [float(m["avg_volume"]) for m in metrics]
        ...
        for i, m in enumerate(metrics):
            score = Decimal("0.5") * Decimal(str(z_caps[i])) + Decimal("0.5") * Decimal(str(z_vols[i]))
