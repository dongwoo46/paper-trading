...
## 목적
- `frontend/trading-web/README.md`의 FSD, React Query, 다크 글래스 UI 규칙을 준수하면서
...
- 과거 데이터 페이지
  - `SymbolCatalogPanel`을 최신 백엔드 응답 스펙으로 전환
...
- 매크로 페이지
  - `FredPanel`을 React Query 기반으로 재구성
    - 카탈로그/선택 목록 API 연동
    - 검색 + 카테고리 + 주기 + 상태 필터
...
## 검증
- `frontend/trading-web`에서 `npm run build` 성공
